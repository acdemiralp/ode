...
public:
  ...
  __device__ __host__ constexpr adaptive_step_iterator& operator++()
  {
    if constexpr (is_extended_butcher_tableau_v<typename method_type::tableau_type>)
    {
      ...
      const auto evaluation = error_evaluator.evaluate(problem.value, step_size, result);
  ...
  __device__ __host__ constexpr adaptive_step_iterator  operator++(std::int32_t)
  {
  ...
  __device__ __host__ friend constexpr bool             operator==(const adaptive_step_iterator& lhs, const adaptive_step_iterator& rhs) noexcept
  {
